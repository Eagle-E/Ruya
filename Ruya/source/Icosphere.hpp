...
namespace ruya
{
	...
	{
	public:
		Icosphere(int level_of_detail = 5, bool oldAlgo = false)
		{
			...
			{
				if (oldAlgo)
					mMeshes[level_of_detail] = init_mesh_old(level_of_detail);
				else 
	...
	private:
		...
		*/
		static std::shared_ptr<Mesh> init_mesh(int levelOfDetail = 5)
		{
			...
			auto add_if_not_added_and_return_index2 = [](vector<vec3>& arr, const vec3& v, unordered_map<vec3, unsigned int, Vec3HashFunction>& addedVecs) -> int
			{
			...
			for (int i = 0; i < levelOfDetail; i++)
			{
				...
				for (uvec3& face : icoMesh->faces)
				{
					...
					unsigned int iv3 = add_if_not_added_and_return_index2(icoMesh->vertices, v3, addedVecs);
					unsigned int iv4 = add_if_not_added_and_return_index2(icoMesh->vertices, v4, addedVecs);
					unsigned int iv5 = add_if_not_added_and_return_index2(icoMesh->vertices, v5, addedVecs);
		...
		static std::shared_ptr<Mesh> init_mesh_old(int levelOfDetail = 5)
		{
			std::shared_ptr<Mesh> icoMesh = Icosahedron::create_icosahedron_mesh();
